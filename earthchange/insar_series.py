...
def export_mintpy(products, run_dir, track):
    ...
    # Only reference bands that are actually present. HyP3 omits the DEM and
    # incidence-angle rasters unless the job asked for them, and a config
    # pointing at absent files fails at load -- after the user has installed
    # MintPy and copied a stack across.
    def present(suffix):
        return any(glob.glob(os.path.join(pdir, f"*{suffix}"))
                   for pdir, _ in products)

    has_dem = present("_dem.tif")
    # _inc_map.tif, not _lv_theta.tif: MintPy wants an incidence angle, and
    # lv_theta is a look-vector component. Note _inc_map.tif is in RADIANS
    # while MintPy's convention is DEGREES -- asc_desc2horz_vert applies
    # deg2rad itself, so an unconverted file yields a clean, plausible, wrong
    # decomposition rather than an error. Convert before decomposing; see
    # scripts/decompose.py, which refuses to run if the values look like radians.
    has_inc = present("_inc_map.tif")

    lines = [
        "# Generated by earthchange -s insar-series.",
        f"# Run:  smallbaselineApp.py {os.path.join(root, 'earthchange.cfg')}",
        "#",
        "# HyP3 INSAR_GAMMA products are already geocoded, so the processor is",
        "# hyp3 and NOT isce.",
    ...
    if has_dem:
        lines.append("mintpy.load.demFile          = ../hyp3/*/*_dem.tif")
    if has_inc:
        # One file, not a glob across every product: incidence angle is a
        # property of the track geometry, not of an individual pair. Globbing
        # all of them would also make MintPy's choice depend on sort order.
        lines.append("mintpy.load.incAngleFile     = ../hyp3/*/*_inc_map.tif")

    lines += [
        "",
        "# These need no geometry, and are the corrections most likely to fix a",
        "# velocity field dominated by atmosphere: a long-wavelength ramp, and",
        "# phase unwrapped wrongly by whole fringes.",
        "mintpy.deramp                     = linear",
        "# Unwrap-error correction is OFF, and this is not a preference.",
        "# BOTH methods need connectComponent, which HyP3 INSAR_GAMMA products",
        "# do not carry. bridging fails loudly (KeyError, hours in, naming an",
        "# HDF5 object rather than the missing input). phase_closure fails",
        "# SILENTLY: it writes an all-zero unwrapPhase_phaseClosure, which",
        "# obsDatasetName=auto then prefers over the good raw phase, and the",
        "# inversion reports 0 pixels out of 1.6 million with no error at all.",
        "# So pin the observed dataset too -- turning the method off is not",
        "# enough while a zeroed dataset is still sitting in the stack.",
        "mintpy.unwrapError.method              = no",
        "mintpy.networkInversion.obsDatasetName = unwrapPhase",
        "# Default 0.7 is unreachable before the tropospheric correction, which",
        "# MintPy runs AFTER this mask is built: over tropical terrain the",
        "# uncorrected median temporal coherence is ~0.06, so 0.7 passes a",
        "# single pixel and aborts the run ahead of the very correction that",
        "# would fix it. 0.1 sits just above the observed p90. It is permissive",
        "# by design -- it lets the run finish, it does not certify the pixels.",
        "# Whether the result is real motion is settled by asc/desc agreement.",
        "mintpy.networkInversion.minTempCoh     = 0.1",
        f"mintpy.reference.minCoherence     = {LOW_COH_FLOOR}",
        f"mintpy.network.tempBaseMax        = {MAX_TEMPORAL_DAYS}",
        "",
    ]
    if has_dem:
        lines += ["mintpy.troposphericDelay.method   = pyaps",
                  "mintpy.topographicResidual        = yes"]
    else:
        lines += [
            "# Tropospheric delay and topographic residual are DISABLED: they",
            "# need *_dem.tif, which these products do not contain because the",
            "# HyP3 jobs were submitted without include_dem. Resubmit with",
            "# include_dem=True and include_inc_map=True to enable them, and to",
            "# decompose ascending and descending into vertical and east-west.",
